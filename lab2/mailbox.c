...
#define VIDEOCORE_MBOX  (MMIO_BASE+0xB880) // start from bus address 0xC000 0000, 0x7E00 0000 - 0xC000 0000 = 
...
void Get_ARM_MEM() {
    mailbox[0] = 8 * 4; // (size in bytes including the header values, the end tag and padding)
    mailbox[1] = MBOX_REQUEST; // buffer request  
    mailbox[2] = MBOX_TAG_GET_ARM_MEMORY; // tag id
    mailbox[3] = 8; // maximum of request and response value buffer's length.
    mailbox[4] = MBOX_TAG_REQUEST_CODE;
    mailbox[5] = 0; // value buffer
    mailbox[6] = 0; // value buffer
    mailbox[7] = MBOX_TAG_LAST; // tags end
    mailbox_call();
}